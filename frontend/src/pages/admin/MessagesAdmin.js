...
const MessagesAdmin = () => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        // Caso 1: Respuesta con estructura {success, data: {messages: [], total: 0}}
  ...
  const toggleStarred = async (id) => {
    try {
      // ContactService no tiene función star por ahora, simular
      console.log('Toggle starred for message:', id);
