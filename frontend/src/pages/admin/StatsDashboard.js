...
// API URLs para compatibilidad  
const AUTH_API_URL = process.env.REACT_APP_AUTH_API_URL || 'http://localhost:8001';
const ANALYTICS_API_URL = process.env.REACT_APP_ANALYTICS_API_URL || 'http://localhost:8007';
...
const statsService = {
  ...
  getAdminStats: async (period = 'month') => {
    try {
      ...
      const response = await fetch(`${ANALYTICS_API_URL}/api/admin/stats?period=${period}`, { 
        method: 'GET',
        headers: headers,
  ...
  getAdminDashboard: async () => {
    try {
      ...
      const response = await fetch(`${ANALYTICS_API_URL}/api/admin/dashboard`, {
        method: 'GET',
        headers: headers,
        credentials: 'include'
      });
  ...
  getSalesStats: async (period = 'month', startDate = null, endDate = null) => {
    try {
      let url = `${ANALYTICS_API_URL}/api/admin/sales-stats?period=${period}`;
  ...
  getActivityLog: async (page = 1, limit = 50, filters = {}) => {
    try {
      let url = `${ANALYTICS_API_URL}/api/admin/activity-log?page=${page}&limit=${limit}`;
...
const StatsDashboard = () => {
  ...
  const checkUserAuth = () => {
    try {
      ...
      fetch(`${AUTH_API_URL}/api/auth/check-admin`, {
        method: 'GET',
        headers: headers,
        credentials: 'include'
      })
