      ...
      ProfileService.getProfile()
        .then(response => {
          if (response.data.success && response.data.data) {
            ...
            const updatedData = {
              name: user.nombre || userData.name,
              email: user.email || userData.email,
              role: user.role === 'admin' ? 'Administrador' : 'Usuario',
              joinDate: formatDate(user.created_at) || userData.joinDate,
              phone: user.phone || '',
              lastLogin: formatDateTime(user.last_login || userData.lastLogin)
            };
            ...
            if (user.image_url) {
              ...
              setProfileImage(`${apiUrl}${user.image_url}`);
            }
          }
        })
        .catch(error => {
