...
class StatsService {
  ...
  getAdminStats(period = 'month') {
    ...
    return axios.get(`${ANALYTICS_API_URL}/api/admin/stats?period=${period}`, { 
      headers: headers,
  ...
  getAdminDashboard() {
    syncUserData(); // Sincronizar datos antes de cada petición
    return axios.get(`${ANALYTICS_API_URL}/api/admin/dashboard`, { 
      headers: authHeader(),
  ...
  checkAdminPermissions() {
    ...
    if (isAdminLocally) {
      return axios.get(`${AUTH_API_URL}/api/auth/check-admin`, { 
        headers: authHeader(),
        withCredentials: true
      })
      .then(response => {
